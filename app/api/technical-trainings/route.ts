...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('POST /api/technical-trainings - Request body:', body);
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Failed to create technical training', details: error.message },
        { status: 500 }
      );
    }

    console.log('POST /api/technical-trainings - Success:', data);
    return NextResponse.json({ data });
  } catch (error) {
    console.error('API error:', error);
    return NextResponse.json(
