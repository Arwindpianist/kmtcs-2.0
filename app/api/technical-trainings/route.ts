...
async function attachCalendarLinks(
  ...
  const ids = records
    ...
    .filter(Boolean);
  const linksByTraining = await getTrainingCalendarLinks('technical_trainings', ids);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = id
      ? await getCatalogRecordById('technical_trainings', id)
      : await listCatalogRecords('technical_trainings', status, limit);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await createCatalogRecord('technical_trainings', trainingPayload);
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Failed to create technical training', details: errorMeta.message },
        { status: 500 }
      );
    }

    if (data?.id && Array.isArray(linked_event_ids)) {
      await setTrainingCalendarLinks('technical_trainings', String(data.id), linked_event_ids);
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { data, error } = await updateCatalogRecord('technical_trainings', id, updateData);
    ...
    if (Array.isArray(linked_event_ids)) {
      await setTrainingCalendarLinks('technical_trainings', id, linked_event_ids);
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { error } = await deleteCatalogRecord('technical_trainings', id);
