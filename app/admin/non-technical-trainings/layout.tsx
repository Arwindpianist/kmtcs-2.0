...
interface NonTechnicalTrainingsLayoutProps {
  children: ReactNode;
}

export default function NonTechnicalTrainingsLayout({ children }: NonTechnicalTrainingsLayoutProps) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        console.log('NonTechnicalTrainingsLayout: Checking authentication...');
        
        // Get current session
        const { data: { session } } = await supabase.auth.getSession();
        
        if (session?.user) {
          console.log('NonTechnicalTrainingsLayout: Session found for user:', session.user.email);
          ...
          console.log('NonTechnicalTrainingsLayout: No session found, redirecting to login...');
          router.push('/login');
        }
      } catch (error) {
        console.error('NonTechnicalTrainingsLayout: Auth check error:', error);
        router.push('/login');
      } finally {
        setIsLoading(false);
      }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        console.log('NonTechnicalTrainingsLayout: Auth state change:', event, session?.user?.email);
          ...
          <div className="flex items-center space-x-4">
            {currentUser && (
              <div className="hidden sm:block text-right">
                ...
                <p className="text-xs text-gray-500">Non-Technical Trainings</p>
              </div>
            )}
          </div>
        </div>
