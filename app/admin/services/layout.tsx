...
export default function ServicesLayout({ children }: ServicesLayoutProps) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        console.log('ServicesLayout: Checking authentication...');
        
        // Get current session
        const { data: { session } } = await supabase.auth.getSession();
        
        if (session?.user) {
          console.log('ServicesLayout: Session found for user:', session.user.email);
          ...
          console.log('ServicesLayout: No session found, redirecting to login...');
          router.push('/login');
        }
      } catch (error) {
        console.error('ServicesLayout: Auth check error:', error);
        router.push('/login');
      } finally {
        setIsLoading(false);
      }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        console.log('ServicesLayout: Auth state change:', event, session?.user?.email);
          ...
          <div className="flex items-center space-x-4">
            {currentUser && (
              <div className="hidden sm:block text-right">
                ...
                <p className="text-xs text-gray-500">Consulting Services</p>
              </div>
            )}
          </div>
        </div>
          ...
          <nav className="p-4 lg:p-6">
            <ul className="space-y-2 lg:space-y-3">
              ...
              <li>
                <Link
                  href="/admin/services"
                  className="flex items-center px-3 py-2 text-purple-700 bg-purple-50 rounded-lg font-medium"
                >
