...
interface ConsultantsLayoutProps {
  children: ReactNode;
}

export default function ConsultantsLayout({ children }: ConsultantsLayoutProps) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        console.log('ConsultantsLayout: Checking authentication...');
        
        // Get current session
        const { data: { session } } = await supabase.auth.getSession();
        
        if (session?.user) {
          console.log('ConsultantsLayout: Session found for user:', session.user.email);
          ...
          console.log('ConsultantsLayout: No session found, redirecting to login...');
          router.push('/login');
        }
      } catch (error) {
        console.error('ConsultantsLayout: Auth check error:', error);
        router.push('/login');
      } finally {
        setIsLoading(false);
      }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        console.log('ConsultantsLayout: Auth state change:', event, session?.user?.email);
            ...
            <ul className="space-y-2 lg:space-y-3">
              ...
              <li>
                <Link
                  ...
                  className="flex items-center px-3 py-2 text-indigo-700 bg-indigo-50 rounded-lg font-medium"
                >
